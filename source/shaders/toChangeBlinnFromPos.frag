...
void main() {
    ...
    // Compute the partial derivatives of the fragment position
	vec3 X = dFdx(fragPos);
    vec3 Y = dFdy(fragPos);
    // Compute the normal vector using the cross product of the partial derivatives
    vec3 N = -normalize(cross(X,Y));

    //--------------- Fetch albedo from texture ---------------
    // Fetch the albedo color from the texture and apply gamma correction
    ...
    for(int i = 0; i < MAX_POINT_LIGHTS; i++) {
        if(gubo.pointLightColor[i].a <= 0.5) 
        ...
        vec3 radianceP = gubo.pointLightColor[i].rgb * attenuation;

        float NdotLp = max(dot(N, Lp), 0.0);
        float HdotNp = max(dot(Hp, N), 0.0);
        // TODO: questo valore (150.0) può essere cambiato: provare altri valori
        vec3 LoP = (albedo * NdotLp + vec3(pow(HdotNp, 150.0))) * radianceP;
        Lo += LoP;
    }
        
    //---------- Compute the ambient component of the lighting ------------
    // Apply a small ambient term (0.015) to simulate indirect lighting
	vec3 ambient = 0.015 * albedo;	
    vec3 color = ambient  + Lo;
    // Reinhard tone mapping to compress the dynamic range of the color
    color = color / (color + vec3(1.0));
    // Apply gamma correction to convert the color from linear space to sRGB space
