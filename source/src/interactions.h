...
public:
    ...
    void reset(Scene &scena)
    {
        ...
        for (auto &pair : doors)
        {
            ...
            if (it != scena.InstanceIds.end())
                scena.I[it->second]->C = door.originalCollider;
        }
    }
    ...
    void updateAnimations(Scene &scena, float deltaT, const glm::vec3 &playerPos)
    {
        for (auto &pair : doors)
        {
            ...
            const glm::mat4 rotation = glm::rotate(
                glm::mat4(1.0f),
                glm::radians(door.currentAngle * door.openSign),
                glm::vec3(0.0f, 1.0f, 0.0f));

            const glm::mat4 nextWorld =
                door.initialWm *
                glm::translate(glm::mat4(1.0f), door.hinge) *
                rotation *
                glm::translate(glm::mat4(1.0f), -door.hinge);

            if (inst->C != nullptr)
            {
                inst->C->setWorldMatrix(nextWorld);

                if (door.currentAngle != previousAngle)
                {
                    const AABBextents bounds = inst->C->getExtents();
                    ...
                    if (overlapsPlayer)
                    {
                        ...
                        door.state = previousState;
                        door.openTimer = previousTimer;
                        continue;
                    }
                }
            }
    ...
    int findNearestInteractable(Scene &scena, const Camera &cam, float maxRange = 4.0f, float aimMargin = 0.15f)
    {
        ...
        for (int i = 0; i < (int)interactables.size(); i++)
        {
            ...
            for (int blockerIndex = 0; blockerIndex < scena.InstanceCount; ++blockerIndex)
            {
                ...
                if (rayIntersectsAABB(ro, rd, blocker->C->getExtents(), blockerDistance) && blockerDistance + 0.001f < t)
                {
                    blocked = true;
                    break;
                }
        ...
        for (int a = 0; a < 3; a++)
        {
            if (fabs(rd[a]) < 1e-6f)
            {
                if (ro[a] < bmin[a] || ro[a] > bmax[a])
                    return false; // parallelo allo slab e fuori
                continue;
            }
            float inv = 1.0f / rd[a];
