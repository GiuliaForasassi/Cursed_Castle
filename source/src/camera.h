...
public:
    ...
    // ------------------ Collision detection -------------------
    // Check if the player (vertical cylinder) collides with any object in the scene
    // The collider of the player before was a sphere, now it's a vertical cylinder
    bool collidesWithScene(glm::vec3 pos, float radius, Scene &scene)
    {
        // Cylinder spans from the ground up to (a bit above) eye height
        const float cylBottom = pos.y - 1.7f; // Bottom of the cylinder
        const float cylTop = pos.y + 0.1f;    // Top of the cylinder
        const glm::vec2 center(pos.x, pos.z); // Center of the cylinder's base in the XZ plane

        // Iterate through all instances in the scene and check for collisions with the cylinder collider
        for (int i = 0; i < scene.InstanceCount; i++)
        {
            ...
            // Check for overlap in the Y axis (vertical) between the cylinder and the AABB of the collider
            if (E.yMax < cylBottom || E.yMin > cylTop)
                continue;
            ...
            // Calculate the square of the distance between the centre of the cylinder and this nearest point, and compare it with the square of the radius
